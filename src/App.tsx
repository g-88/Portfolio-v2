...
import { projects, operatingPrinciples, educationAndDetails } from "./data";
import { Project, OperatingPrinciple, RuleInput, RuleEvaluationResult } from "./types";
...
export default function App() {
  ...
  const [selectedProject, setSelectedProject] = useState<string>("bitcoin-depot");
  ...
  const [sandboxKyc, setSandboxKyc] = useState<"unverified" | "phone_verified" | "id_verified">("phone_verified");
  ...
  const [sandboxResult, setSandboxResult] = useState<RuleEvaluationResult | null>(null);
  ...
  const applySandboxScenario = (amount: number, kyc: "unverified" | "phone_verified" | "id_verified", prov: string, pep: boolean) => {
  ...
  const runRuleEngine = () => {
    ...
    setTimeout(() => {
      ...
      pushLog(`Payload loaded: amount=${sandboxAmount}, kyc=${sandboxKyc}, province=${sandboxProvince}, PEP=${sandboxPEP}`);
      pushLog(`Verifying active rule schemas corresponding to province constraint: ${sandboxProvince}`);

      // Evaluate Rule 1: Restricted Party AML Screening
      let r1Result: "SKIPPED" | "PASSED" | "FAILED" = "PASSED";
      if (sandboxPEP) {
        r1Result = "FAILED";
        pushLog("Rule Match: CRITICAL_PEP_ALERT triggers. Politically Exposed Persons list triggers override check.");
      } else {
      ...
      if (sandboxAmount >= 9000) {
        ...
        pushLog(`Rule Match: MAX_TRANSACTION_CEILING_EXCEEDED (Amount $${sandboxAmount} exceeds $9,000 limit)`);
        ...
        pushLog(`Rule Match: KYC_ELEVATION_REQUIRED (Present KYC: ${sandboxKyc})`);
        ...
        pushLog(`Rule Match: PHONE_AUTHENTICATION_REQUIRED (Present KYC: unverified)`);
      } else {
        pushLog("Rule Pass: Transaction amount sits within boundaries for present verification states.");
      }

      // Evaluate Rule 3: Geographic Restrict list (Regulatory compliance)
      let r3Result: "SKIPPED" | "PASSED" | "FAILED" = "PASSED";
      let r3Msg = "";
      pushLog(`Evaluating state restriction schema: 'verify_geographic_rules' for ${sandboxProvince}`);
      
      // Suppose NY and QC have restricted rules
      if (sandboxProvince === "NY" && sandboxAmount > 1000) {
        r3Result = "FAILED";
        r3Msg = "New York BitLicense rules impose maximum anonymous limits of $1000 in local operations.";
        pushLog("Rule Match: GEOGRAPHIC_COMPLIANCE_LIMIT (NY BitLicense limitation triggered on active amount)");
      } else {
        pushLog(`Rule Pass: Geographic jurisdiction validation accepted for ${sandboxProvince}.`);
      }

      // Final aggregation
      let finalAction: "APPROVE" | "USER_ID_PROMPT" | "BLOCK_TRANSACTION" | "ESCALATE_MANUAL" = "APPROVE";
      let finalReason = "Transaction fits within risk profile. Approved to advance to cashier payout queue.";

      if (sandboxPEP) {
        finalAction = "BLOCK_TRANSACTION";
        finalReason = "Compliance block. Politically Exposed Person match detected. Transaction aborted and reported to FINTRAC logs.";
      ...
      pushLog(`Result: ${finalAction} | Code: ${finalAction === "APPROVE" ? "200" : "403"}`);
      ...
      setSandboxResult({
        ...
        reason: finalReason,
        stepsEvaluated: [
          { ruleName: "CRITICAL_PEP_ALERT", result: r1Result, log: sandboxPEP ? "Matched sanction restriction list." : "No sanction matches found." },
          { ruleName: "VALIDATE_AML_THRESHOLDS", result: r2Result, log: r2Msg || "Transaction value within boundaries." },
          { ruleName: "VERIFY_GEOGRAPHIC_RULES", result: r3Result, log: r3Msg || "No geographic exceptions triggered." }
        ]
      });
  ...
  return (
    <div className="min-h-screen bg-brand-obsidian text-brand-silver font-sans relative overflow-x-hidden antialiased selection:bg-brand-mint selection:text-brand-obsidian">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-brand-obsidian/85 border-b border-brand-slate py-4 px-6 md:px-12 transition-all">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <a href="#operating-style" className="font-mono text-xs text-brand-concrete hover:text-brand-mint transition-colors tracking-widest uppercase">
              // 01. OPERATING STYLE
            </a>
            <a href="#regulatory-sandbox" className="font-mono text-xs text-brand-concrete hover:text-brand-mint transition-colors tracking-widest uppercase">
              // 02. COMPLIANCE SANDBOX
            </a>
            <a href="#featured-work" className="font-mono text-xs text-brand-concrete hover:text-brand-mint transition-colors tracking-widest uppercase">
              // 03. FEATURED WORK
            </a>
            <a href="#contact" className="font-mono text-xs text-brand-concrete hover:text-brand-mint transition-colors tracking-widest uppercase">
              // 04. CONTACT/PING
            </a>
          </nav>
          ...
          <div className="hidden lg:flex items-center gap-5 border-l border-brand-slate pl-6 font-mono text-xs text-brand-concrete">
            <div className="flex items-center gap-2">
              ...
              <span className="text-brand-silver font-semibold tracking-wider text-[11px]">STATUS: RESILIENT</span>
            </div>
            ...
            <div className="flex flex-col gap-5 font-mono text-sm">
              ...
              <div className="pt-4 border-t border-brand-slate flex flex-col gap-3 text-xs text-brand-concrete">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-brand-silver">System: SECURE & READY</span>
                </div>
                <div className="flex items-center gap-2">
                  <Clock size={12} className="text-brand-mint" />
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 py-12 relative z-10">
        ...
        {/* HERO SECTION */}
        <section className="min-h-[80vh] flex flex-col justify-center py-12 md:py-20" id="hero">
            ...
            <div className="lg:col-span-7 space-y-8">
              ...
              >
                ...
                <span className="font-mono text-[9px] tracking-widest text-brand-mint uppercase font-semibold">
                  AVAILABLE FOR NEW PRODUCTION WORK · OTTAWA, CANADA
                </span>
              </motion.div>
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <span className="italic font-serif text-brand-mint font-normal text-5xl sm:text-6.5xl lg:text-[76px] tracking-tight block my-1">complex, messy</span> 
                  workflows.
                </motion.h1>
                ...
                >
                  What sets me apart is a cross-domain fusion of technical judgment, communication, and real-world operational reliability across regulated fintech, custom automated flows, and production rollouts.
                </motion.p>
              </div>
              ...
              >
                ...
                <a href={educationAndDetails.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-brand-mint transition-colors">
                  <Linkedin size={14} /> /griffinleblanc
                </a>
                <a href={educationAndDetails.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-brand-mint transition-colors">
                  <Github size={14} /> /g-88
                </a>
                <a href={`mailto:${educationAndDetails.email}`} className="flex items-center gap-1 hover:text-brand-mint transition-colors">
              ...
              <div className="relative bg-brand-coal border border-brand-slate rounded-xl overflow-hidden shadow-2xl">
                ...
                <div className="bg-brand-slate/40 border-b border-brand-slate px-4 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-brand-concrete ml-2">griffin-production-agent.sh</span>
                  </div>
                ...
                <div className="p-6 space-y-6 font-mono text-xs">
                  ...
                  <div className="space-y-2 border-b border-brand-slate/40 pb-5">
                    <p className="text-brand-concrete">// PRODUCTION STACK VALIDATED</p>
                    <div className="grid grid-cols-2 gap-3 text-[11px]">
                      <div className="bg-brand-slate/30 p-2.5 rounded border border-brand-slate/50">
                        <span className="text-brand-concrete block text-[10px]">LATEST ROLLOUT</span>
                        <span className="text-brand-mint font-semibold">100% SUCCESS RATE</span>
                      </div>
                      <div className="bg-brand-slate/30 p-2.5 rounded border border-brand-slate/50">
                        <span className="text-brand-concrete block text-[10px]">RELIABILITY</span>
                        <span className="text-brand-silver font-semibold">ZERO COMPLIANCE DRIFT</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 pb-5 border-b border-brand-slate/40">
                    <div className="flex justify-between items-center text-[11px]">
                      <span className="text-brand-concrete text-xs flex items-center gap-1.5">
                        ...
                        Kiosk Rules Speed (json-rules-engine)
                      </span>
                      <span className="text-brand-mint-accent">&lt;2.4ms avg</span>
                    </div>
                    {/* Fake visual bar */}
                    <div className="w-full h-1.5 bg-brand-slate rounded-full overflow-hidden">
                      <div className="bg-brand-mint h-full rounded-full w-[94%]" />
                    </div>

                    <div className="flex justify-between items-center text-[11px]">
                      <span className="text-brand-concrete text-xs flex items-center gap-1.5">
                        <Database size={14} className="text-brand-mint" />
                        Database Index Query Time
                      </span>
                      <span className="text-brand-mint-accent">1.1ms</span>
                    </div>
                    <div className="w-full h-1.5 bg-brand-slate rounded-full overflow-hidden">
                      <div className="bg-brand-mint h-full rounded-full w-[98%]" />
                    </div>

                    <div className="flex justify-between items-center text-[11px]">
                      <span className="text-brand-concrete text-xs flex items-center gap-1.5">
                        <Activity size={14} className="text-brand-mint" />
                        Lighthouse performance on Faceout
                      </span>
                      <span className="text-brand-mint-accent">100 / 100</span>
                    </div>
                    <div className="w-full h-1.5 bg-brand-slate rounded-full overflow-hidden">
                  ...
                  <div className="space-y-1 text-brand-concrete text-[10.5px]">
                    <p className="text-brand-mint">$ npx deploy-agent generate --profile=griffin</p>
                    <p className="text-brand-silver">✓ Extracted: 3 Core Operating Philosophies</p>
                    <p className="text-brand-silver">✓ Loaded: Bitcoin Depot, PermiPro, Faceout datasets</p>
                    <p className="text-brand-silver">✓ Compiled compliance_rules_verifier.py [OK]</p>
                    <p className="text-[#34D399] animate-pulse-slow">&gt; SYSTEM RUNTIME STEADY. WAITING FOR COMPLIANCE INPUTS...</p>
                  </div>
        ...
        <section className="py-20 border-t border-brand-slate" id="operating-style">
          <div className="space-y-10">
            
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div className="space-y-2">
                <span className="font-mono text-xs text-brand-mint tracking-wider uppercase">// 01. CRITICAL PHILOSOPHY</span>
                ...
                <p className="text-brand-concrete max-w-xl text-sm">
                  I'm at my best when features cross boundaries. I do not just write decoupled blocks of code; I own the implementation, rollout, and downstream environment constraints.
                </p>
              </div>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1">
                          <h3 className={`font-display font-semibold text-base ${
                            isSelected ? "text-white" : "text-brand-silver group-hover:text-white transition-colors"
                          }`}>
                            {principle.title}
                          </h3>
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-brand-concrete text-sm leading-relaxed">
                        Below is a real-world engineering challenge faced during client/firm projects, summarizing Griffin's response, system refactor choice, and downstream results.
                      </p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        ...
                        <div className="bg-brand-slate/20 border border-brand-slate/40 p-4 rounded-none space-y-2">
                          ...
                          <p className="text-xs text-brand-silver leading-relaxed">
                            {operatingPrinciples[selectedPrinciple].caseStudy.problem}
                          </p>
                        </div>
                        ...
                        <div className="bg-brand-slate/20 border border-brand-slate/40 p-4 rounded-none space-y-2">
                          ...
                          <p className="text-xs text-brand-silver leading-relaxed">
                            {operatingPrinciples[selectedPrinciple].caseStudy.solution}
                          </p>
                        </div>
                    ...
                    <div className="bg-brand-mint/5 border border-brand-mint/20 rounded-none p-4 flex items-center justify-between">
                      <div className="space-y-1">
                        ...
                        <span className="text-xs text-white antialiased">
                          {operatingPrinciples[selectedPrinciple].caseStudy.impact}
                        </span>
                      </div>
        ...
        <section className="py-20 border-t border-brand-slate relative" id="regulatory-sandbox">
          <div className="space-y-10">
            
            <div className="space-y-2">
              <span className="font-mono text-xs text-brand-mint tracking-wider uppercase">// 02. INTERACTIVE DEMO</span>
              <h2 className="font-display text-3xl font-bold tracking-tight text-white">
                Interactive State Rules Compliance Sandbox
              </h2>
              <p className="text-brand-concrete max-w-2xl text-sm">
                At Bitaccess/Bitcoin Depot, Griffin created and optimized strict state-based regulatory checking structures with <code className="font-mono text-brand-mint bg-brand-coal px-1.5 py-0.5 rounded text-xs">json-rules-engine</code>. Match variables below to see an exact client-side model of how regional regulations are computed dynamically at cold physical kiosks.
              </p>
            </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <button
                        type="button"
                        onClick={() => applySandboxScenario(450, "unverified", "ON", false)}
                      ...
                      <button
                        type="button"
                        onClick={() => applySandboxScenario(2500, "phone_verified", "AB", false)}
                        className="font-mono text-[10px] bg-brand-slate/45 hover:bg-brand-slate border border-brand-slate hover:border-brand-mint/30 text-brand-silver rounded py-1.5 text-center transition-all"
                      >
                        KYC Flow Trigger
                      </button>
                      <button
                        type="button"
                        onClick={() => applySandboxScenario(1200, "unverified", "NY", true)}
                        className="font-mono text-[10px] bg-brand-slate/45 hover:bg-brand-slate border border-brand-slate hover:border-brand-mint/30 text-brand-silver rounded py-1.5 text-center transition-all"
                      >
                        Sanctions Block
                      </button>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(["unverified", "phone_verified", "id_verified"] as const).map((kyc) => (
                        <button
                          key={kyc}
                          type="button"
                          onClick={() => {
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <span className={`h-2.5 w-2.5 rounded-full ${sandboxPEP ? "bg-red-500 animate-pulse" : "bg-brand-slate"}`} />
              ...
              <div className="lg:col-span-7 flex flex-col gap-6">
                ...
                <div className="bg-brand-coal border border-brand-slate rounded-xl overflow-hidden shadow-xl flex flex-col h-full">
                  ...
                  <div className="p-4 bg-[#0a0c0e] font-mono text-[10.5px] leading-relaxed flex-1 overflow-auto max-h-[220px]">
                    <span className="text-brand-concrete font-sans text-xs italic block mb-2">// Declared active JSON regulatory constraints:</span>
  ...
  "event": {
    ...
    "params": { 
      ...
      "reason": "Governments enforce full user ID checks past limits."
                  ...
                  <div className="border-t border-brand-slate p-5 space-y-4 bg-brand-coal">
                    ...
                    <div className="bg-brand-obsidian/80 border border-brand-slate p-4 rounded-lg font-mono text-[11px] space-y-1.5 text-brand-concrete max-h-[140px] overflow-y-auto">
                      {sandboxLogs.map((log, idx) => (
                        <p key={idx} className={log.includes("Rule Match") || log.includes("Rule FAILED") ? "text-yellow-400" : log.includes("Rule Pass") ? "text-brand-mint" : "text-brand-concrete"}>
                          {log}
                        </p>
                      ))}
                      {sandboxIsEvaluating && (
                        <p className="text-brand-mint animate-pulse-slow">&gt; Simulating network query feedback packet...</p>
                      )}
                    </div>
                    ...
                    {sandboxResult && (
                      ...
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className={`font-mono text-[10px] font-bold px-2 py-0.5 rounded tracking-wide uppercase ${
                              sandboxResult.passed 
                              ...
                              STATUS: {(sandboxResult.actionRequired).replace("_", " ")}
                            </span>
                        ...
                        <div className="space-y-1 border-t md:border-t-0 md:border-l border-brand-slate/50 pt-3 md:pt-0 md:pl-5 shrink-0 font-mono text-[10px] flex flex-col gap-1 text-brand-concrete">
                          {sandboxResult.stepsEvaluated.map((step) => (
                            <div key={step.ruleName} className="flex items-center gap-2">
                              ...
                              <span>{step.ruleName}: <b className={
                                step.result === "PASSED" 
                                  ? "text-brand-mint" 
                                  : step.result === "FAILED"
                                  ? "text-red-400"
        ...
        <section className="py-20 border-t border-brand-slate" id="featured-work">
          <div className="space-y-10">
            
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div className="space-y-2">
                <span className="font-mono text-xs text-brand-mint tracking-wider uppercase">// 03. FEATURED PROJECTS</span>
                <h2 className="font-display text-3xl font-bold tracking-tight text-white mb-2">
                  What I've been building lately.
                </h2>
                <p className="text-brand-concrete max-w-xl text-sm">
                  A look at high-stakes fintech software, complex regulatory code integration, enterprise municipality tools, and Ottawa business accelerators.
                </p>
              </div>
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="space-y-3">
                          <span className="font-mono text-[10px] text-brand-concrete uppercase block tracking-wider">
                            Griffin's Direct Engineering Impact:
                          </span>
                          <ul className="space-y-2.5 text-xs text-brand-silver font-sans">
                            {proj.highlights.map((hlt, i) => (
                              <li key={i} className="flex items-start gap-2.5">
                                ...
                                <span className="leading-relaxed leading-6">{hlt}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
                    ...
                    <div className="lg:col-span-5 bg-brand-slate/20 border-t lg:border-t-0 lg:border-l border-brand-slate p-6 md:p-8 flex flex-col justify-between space-y-8">
                      ...
                      <div className="bg-brand-coal border border-brand-slate rounded-lg p-6 text-center space-y-2 relative overflow-hidden group">
                        ...
                        <p className="font-sans text-[11px] text-brand-concrete font-medium pt-1">
                          Audited production deployment parameters
                        </p>
                      </div>
                      ...
                      <div className="space-y-3 border-t border-brand-slate/30 pt-6">
                        ...
                        <div className="bg-brand-obsidian/60 border border-brand-slate/55 p-4 rounded font-mono text-[11px] text-brand-concrete space-y-2">
                          {proj.id === "bitcoin-depot" && (
                            <>
                              <p className="text-brand-mint">// Highly regulated transactions pipeline</p>
                              <p className="text-[10px] leading-relaxed">Built state validation using declarative compliance checks, moving logic out of monolith codebases. Safeguarded rollout reliability against poor regional network links in remote field cash point kiosks.</p>
                            </>
                          )}
                          {proj.id === "permipro" && (
                            <>
                              <p className="text-brand-mint">// High-trust administrative portal</p>
                              <p className="text-[10px] leading-relaxed">Multi-role permissions matrices controlling document upload, municipality approvals, workflow transition states, and audit trails. Scaled cleanly with automatic PDF and document processors.</p>
                            </>
                          )}
                          {proj.id === "faceout" && (
                            <>
                              <p className="text-brand-mint">// Extreme Performance Static Engine</p>
                              <p className="text-[10px] leading-relaxed">Leverages Astro static builders, full layout stabilization, zero client-JS overhead by default, and tailored regional indexing heuristics.</p>
                            </>
                          )}
                        </div>
                      </div>
        ...
        <section className="py-20 border-t border-brand-slate" id="contact">
          <div className="space-y-10">
            ...
            <div className="space-y-2">
              <span className="font-mono text-xs text-brand-mint tracking-wider uppercase">// 04. ENDPOINT TRANSMISSION</span>
              <h2 className="font-display text-3xl font-bold tracking-tight text-white animate-pulse-slow">
                Initiate Communication Ping
              </h2>
              <p className="text-brand-concrete max-w-xl text-sm">
                Recruiters, founders, or product heads: submit a dynamic payload below. The message will trigger a simulated API submission and append directly to the client-side session telemetry list.
              </p>
            </div>
                ...
                <form onSubmit={handleSendMessage} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      ...
                      placeholder="e.g. rachel@fintrackiosk.com"
                      className="w-full bg-brand-slate border border-brand-slate hover:border-brand-slate/80 text-brand-silver text-xs rounded p-2.5 focus:ring-1 focus:ring-brand-mint focus:outline-none placeholder:text-brand-concrete/50"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      ...
                      placeholder="e.g. Bitcoin Depot Inc."
                      className="w-full bg-brand-slate border border-brand-slate hover:border-brand-slate/80 text-brand-silver text-xs rounded p-2.5 focus:ring-1 focus:ring-brand-mint focus:outline-none placeholder:text-brand-concrete/50"
                    />
                  </div>
                ...
                <div className="bg-brand-slate/10 border border-brand-slate rounded-xl p-6 md:p-8 space-y-6">
                  ...
                  <div className="space-y-4">
                    <p className="text-brand-concrete text-xs">
                      Live interaction log tracking current session events (stored stateful in client local memory):
                    </p>
                    ...
                    <div className="bg-brand-obsidian border border-brand-slate rounded-lg p-5 font-mono text-[11px] text-brand-concrete space-y-3 max-h-[310px] overflow-y-auto">
                      ...
                      {messagesLog.length === 0 ? (
                        <div className="text-center py-10 space-y-2 text-brand-concrete/60">
                          <AlertCircle size={20} className="mx-auto text-brand-concrete/40" />
                          <p>No messages initialized yet.</p>
                          <p className="text-[10px]">Use form on left to POST simulated API entries.</p>
                        </div>
                      ) : (
                        messagesLog.map((msg) => (
                          <motion.div 
                          ...
                          >
                            <div className="flex items-center justify-between text-[10px] border-b border-brand-slate/20 pb-1.5 mb-1.5">
                              <span className="text-brand-mint font-semibold">{msg.id} (STATUS: 200 OK)</span>
                              <span>{msg.timestamp}</span>
                            </div>
                            <p className="text-[11px] text-brand-silver font-semibold">
                              Payload Sender: <span className="text-brand-mint-accent">{msg.sender} ({msg.company})</span>
                            </p>
                            <p className="text-brand-silver leading-relaxed text-[10.5px]">
                              Body: <span className="text-brand-silver">"{msg.payload}"</span>
                            </p>
                          </motion.div>
                        ))
                      )}
                      ...
                      <div className="border-t border-brand-slate/20 pt-3 text-[10px] text-brand-concrete space-y-1">
                        <p className="text-brand-concrete/50">[{new Date().toLocaleTimeString()}] SECURE Ingress connection validated. User session secure.</p>
                        <p className="text-brand-concrete/50">[{new Date().toLocaleTimeString()}] Local time syncing. Static Astro builds fully optimized.</p>
                      </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="space-y-1 text-center md:text-left">
            ...
            <p className="font-mono text-[11px]">
              Ottawa, ON · Full-stack Product Engineer
            </p>
          </div>
          ...
          <div className="hidden lg:block text-center font-mono text-[10px] text-brand-concrete bg-brand-slate/20 px-3 py-1.5 rounded border border-brand-slate">
            System checksum compiled: <span className="text-brand-mint">SHA-256:7B8F9A</span> | Production Certified [OK]
          </div>

          <div className="flex items-center gap-6 font-mono text-[11px]">
            <a href={educationAndDetails.linkedin} target="_blank" rel="noopener noreferrer" className="hover:text-[#0077b5] transition-colors flex items-center gap-1">
              <Linkedin size={12} /> LinkedIn
            </a>
            <a href={educationAndDetails.github} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors flex items-center gap-1">
              <Github size={12} /> GitHub
            </a>
            <a href={educationAndDetails.resumeUrl} target="_blank" rel="noopener noreferrer" className="hover:text-brand-mint transition-colors flex items-center gap-1">
